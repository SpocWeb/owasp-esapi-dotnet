...
namespace Owasp.Esapi
{
	...
	{
		static readonly Dictionary<string, Dictionary<string, Event>> users =
		...
		readonly Dictionary<string, Threshold> _thresholds;
		...
		public void AddThreshold(Threshold threshold)
		{
			...
			if (_thresholds.ContainsKey(threshold.Event)) throw new ArgumentException();

			// Validate all required actions have been registered already
			if (threshold.Actions != null)
				foreach (var name in threshold.Actions)
					if (!_actionManager.Contains(name))
					{
						var message = string.Format(EM.IntrusionDetector_ActionNotFound1, name);
		...
		public void AddException(Exception e)
		{
			...
			if (e is IntrusionException) return;
		...
		/// <param name="eventName"></param>
		/// <returns></returns>
		Threshold GetEventThreshold(string eventName)
		{
			Threshold threshold;
			_thresholds.TryGetValue(eventName, out threshold);

			// Event not found, create default
			if (threshold == null) threshold = new Threshold(eventName, 0, 0, null);
		...
		/// </summary>
		/// <param name="eventName"></param>
		void OnIntrusionDetected(string eventName, IntrusionException e)
		{
			Debug.Assert(e != null);

			var quota = GetEventThreshold(eventName);
			if (quota == null) throw new ArgumentException(EM.IntrusionDetector_UnknownEventName, "eventName");

			// Take actions
			foreach (var action in quota.Actions)
			{
				// Log action execution
				var message = string.Format(EM.InstrusionDetector_ExceededQuota4, quota.MaxOccurences,
					quota.MaxTimeSpan, eventName, action);
		...
		/// </param>
		void AddSecurityEvent(string eventName)
		{
			var currentUser = Esapi.SecurityConfiguration.CurrentUser;
			var username = currentUser != null && currentUser.Identity != null
			...
			var q = GetEventThreshold(eventName);
			Debug.Assert(q != null);

			if (q.MaxOccurences > 0) securityEvent.Increment(q.MaxOccurences, q.MaxTimeSpan);
		}
		...
		class ActionManager
		{
			readonly List<string> _actions = new List<string> {"log", "disable", "logout"};
