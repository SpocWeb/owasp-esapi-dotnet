...
namespace Owasp.Esapi
{
	...
	public class HttpUtilities : IHttpUtilities
	{
		...
		{
			var csrfToken = (string) HttpContext.Current.Session[CSRF_TOKEN_NAME];
			if (csrfToken == null)
			{
			...
			var token = CSRF_TOKEN_NAME + "=" + csrfToken;
		...
		{
			var csrfToken = (string) HttpContext.Current.Session[CSRF_TOKEN_NAME];
			var receivedCsrfToken = HttpContext.Current.Request.QueryString[CSRF_TOKEN_NAME];
		...
		public void LogHttpRequest(HttpRequest request, ILogger logger, ICollection<string> obfuscatedParams)
		{
			...
			{
				var writer = new HttpRequestWriter(dataWriter);
