...
namespace Owasp.Esapi.Runtime
{
	...
	internal class RuntimeEventBridge : IRuntimeEventListener, IRuntimeEventPublisher
	{
		...
		protected virtual bool BeforeForwardEvent(EventHandler<RuntimeEventArgs> handler, object sender,
		...
		protected virtual bool AfterForwardEvent(EventHandler<RuntimeEventArgs> handler, object sender,
			RuntimeEventArgs args)
		{
			return false;
		}
		...
		protected virtual bool ForwardEventFault(EventHandler<RuntimeEventArgs> handler, object sender,
		...
		void ForwardEvent(EventHandler<RuntimeEventArgs> handler, object sender, RuntimeEventArgs args)
		{
			if (handler != null)
				try
				{
					if (BeforeForwardEvent(handler, sender, args)) return;

					handler(sender, args);

					if (AfterForwardEvent(handler, sender, args)) return;
				}
				catch (Exception exp)
				{
					if (!ForwardEventFault(handler, sender, args, exp)) throw;
				}
		}
		...
		/// <param name="sender"></param>
		/// <param name="args"></param>
		void OnPreRequestHandlerExecute(object sender, RuntimeEventArgs args)
		{
		...
		/// <param name="sender"></param>
		/// <param name="args"></param>
		void OnPostRequestHandlerExecute(object sender, RuntimeEventArgs args)
		{
