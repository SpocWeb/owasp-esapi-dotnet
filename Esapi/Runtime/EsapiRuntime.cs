...
namespace Owasp.Esapi.Runtime
{
	...
	{
		readonly NamedObjectRepository<IAction> _actions;
		readonly NamedObjectRepository<ICondition> _conditions;
		readonly NamedObjectRepository<IContext> _contexts;

		readonly ReaderWriterLockSlim _contextsLock;
		readonly NamedObjectRepository<IRule> _rules;
		...
		public void Dispose()
		{
			...
			try
			{
				foreach (var context in _contexts.Objects)
				{
					var ctxDispose = context as IDisposable;
					if (ctxDispose != null) ctxDispose.Dispose();
				}
			}
			finally
			{
		...
		public override void Unsubscribe(IRuntimeEventPublisher publisher)
		{
			...
			try
			{
				foreach (var context in _contexts.Objects)
				{
					var rteListener = context as IRuntimeEventListener;
		...
		/// </summary>
		public IObjectRepository<string, IAction> Actions => _actions;

		/// <summary>
		///     Runtime registered rules
		/// </summary>
		public IObjectRepository<string, IRule> Rules => _rules;

		/// <summary>
		///     Runtime registered conditions
		/// </summary>
		public IObjectRepository<string, ICondition> Conditions => _conditions;

		/// <summary>
		///     Context hierarchy
		/// </summary>
		public ICollection<IContext> Contexts => _contexts.Objects;
		...
		public IContext CreateContext(string name)
		{
			...
			try
			{
				...
				if (_contexts.Lookup(name, out prevContext)) throw new ArgumentException();
		...
		public void RegisterContext(string name, IContext context)
		{
			...
			try
			{
				...
				if (context is IRuntimeEventListener) ((IRuntimeEventListener) context).Subscribe(this);
				_contexts.Register(name, context);
			}
			finally
			{
		...
		public IContext RemoveContext(string name)
		{
			...
			{
				IContext context;
				if (_contexts.Lookup(name, out context)) _contexts.Revoke(name);
				return context;
			}
			finally
			{
