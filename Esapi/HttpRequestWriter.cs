...
namespace Owasp.Esapi
{
	...
	internal class HttpRequestWriter : HttpDataWriter
	{
		...
		internal void Write(HttpRequest request, IPrincipal user, ICollection<string> obfuscatedParams, bool verbose)
		{
			...
			var userPrincipal = user == null ? Esapi.SecurityConfiguration.CurrentUser : user;
			...
			foreach (HttpCookie cookie in request.Cookies) WriteValue(cookie.Name, cookie.ToString());
