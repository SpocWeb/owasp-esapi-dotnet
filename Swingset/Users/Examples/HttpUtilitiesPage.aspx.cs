...
namespace Owasp.Esapi.Swingset.Users.Examples
{
	public partial class HttpUtilitiesPage : SwingsetPage
	{
		protected void Page_Load(object sender, EventArgs e)
		{
			...
			var length = Request.Url.ToString().IndexOf('?');
			if (length < 0) length = Request.Url.ToString().Length;
			var url = Esapi.HttpUtilities.AddCsrfToken(Request.Url.ToString().Substring(0, length));
