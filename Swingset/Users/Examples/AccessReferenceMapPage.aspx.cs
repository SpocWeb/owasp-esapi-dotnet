		...
		protected void Page_Load(object sender, EventArgs e)
		{
			...
			if (id != null)
			{
				if (!Esapi.Validator.IsValid(BuiltinValidationRules.Printable, Request.QueryString["id"]))
					throw new ValidationException("The parameter value supplied was not valid.",
						"The ID parameter to the AccessReferenceMap page was not valid.");
				var account = ((AccountMapper) Session["AccountMapper"]).GetAccountFromReference(id);
